...
try {
  ...
  // 大屏（企业总数=5000、榜单=真实企业）
  await p.goto(URL + '/park-screen', { waitUntil: 'domcontentloaded' })
  await p.waitForTimeout(8000)
  await p.screenshot({ path: `${EV}/screen_5000.png` })
  console.log('大屏 done')

  // 会员管理（5000 条）
  await p.goto(URL + '/enterpriseManage/memberMgSys', { waitUntil: 'domcontentloaded' })
  await p.waitForTimeout(4000)
  await p.screenshot({ path: `${EV}/member_5000.png` })
  console.log('会员管理 done, 行数:', await p.locator('.el-table__row').count())
} catch (e) {
